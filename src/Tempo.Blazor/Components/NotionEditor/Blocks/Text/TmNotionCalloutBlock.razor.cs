...
/// <summary>
/// Self-contained callout block: emoji icon (with picker) + contenteditable body.
/// Owns its JS keyboard-handler lifecycle, identical to TmNotionTextBlock pattern.
/// [JSInvokable] names must match notion-editor.js; EventCallback names are distinct
/// to avoid C# property/method conflicts.
/// </summary>
public partial class TmNotionCalloutBlock : ComponentBase, IAsyncDisposable
{
    // ── DI ───────────────────────────────────────────────────────────────────
    ...
    [Parameter] public ICalloutBlockContent? Content     { get; set; }
    ...
    /// <summary>Fired when a markdown pattern is recognised. Arg = shortcut key.</summary>
    [Parameter] public EventCallback<string>                    OnTypeConvert     { get; set; }

    /// <summary>Fired when the editable body receives focus.</summary>
    ...
    private DotNetObjectReference<TmNotionCalloutBlock>?  _dotNetRef;
    private bool                                          _kbInitialized;
    private bool                                          _dirty;
    private string?                                       _lastHtml;
    private ICalloutBlockContent?                         _lastContent;
    ...
    private async Task HandleEmojiSelectedAsync(string emoji)
    {
        _showEmojiPicker = false;
        await OnEmojiChanged.InvokeAsync(emoji);
    }

    private async Task HandleEmojiRemovedAsync()
    {
        _showEmojiPicker = false;
