...
export async function loadReportingFonts(fontFaces = []) {
...
export async function paintReportingSnapshot(canvas, snapshot, options = {}) {
...
export async function measureReportingSamples(samples = []) {
...
function normalizeSnapshot(snapshot) {
    ...
    return {
        ...
        pages: Array.isArray(source.pages || source.Pages)
            ? (source.pages || source.Pages).map(page => ({
                ...
                commands: Array.isArray(page.commands || page.Commands)
                    ? (page.commands || page.Commands).map(normalizeCommand)
                    : [],
            }))
            : [],
    };
