...
public static class WireframeDocumentTools
{
    ...
    [Description("Create a new empty wireframe document with the given title and return its id and modifiedAt. Then use wireframe_apply_operations or wireframe_replace_document to build it. Pass scopeAppId (app GUID) when your API key grants access to more than one application, so the new wireframe is created in the intended app.")]
    public static async Task<string> CreateDocument(
        ITempoDocumentLibraryProvider library,
        IWireframeDocumentProvider documents,
        [Description("Title for the new wireframe.")] string title,
        [Description("Optional app id (GUID) scoping the new wireframe; required when the API key grants access to more than one app.")] string? scopeAppId = null)
    {
        var (id, _) = await documents.CreateWireframeDocumentAsync(title, scopeAppId);
