...
public static class DiagramDocumentTools
{
    ...
    [Description("Create a new empty diagram/draw document with the given title and return its id and modifiedAt. Then use diagram_apply_operations or diagram_replace_document to build it.")]
    public static async Task<string> CreateDocument(
        ITempoDocumentLibraryProvider library,
        IDiagramDocumentProvider documents,
        [Description("Title for the new diagram/draw document.")] string title)
    {
        var (id, document) = await documents.CreateDiagramDocumentAsync(title);
